...
from .base import BaseExperiment, DataParallelDistribution
...
class DiffusionExperiment(BaseExperiment):
    ...
    def __init__(self, res):
        ...
        optim_id = res['optim_id']
        train_loader = res['train_loader']
        eval_loader = res['eval_loader']
        model = res['model']
        optimizer = res['optimizer']
        scheduler_iter = res['scheduler_iter']
        scheduler_epoch = res['scheduler_epoch']
        
        if args.log_home is None:
            self.log_base = os.path.join(HOME, 'results', 'flow')
            print(self.log_base)
        else:
