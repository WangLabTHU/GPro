...
class SSWM():
    ...
    def maximize_next_generation(self, population_current, population_current_fitness): 
        
        population_next_all = self.population_mutator(list(population_current))
        population_next_all_predictions = self.evaluate(list(population_next_all))
        ...
        for i in tqdm(range(len(population_current))) :  
            max_score = -np.inf
            for j in range(3*self.seq_len) : 
                if (population_next_all_predictions[3*self.seq_len*i + j] > max_score) :
                    population_next_max_seq[i] = population_next_all[3*self.seq_len*i + j]
                    population_next_max_seq_fitness[i] = population_next_all_predictions[3*self.seq_len*i + j]
                    max_score = population_next_all_predictions[3*self.seq_len*i + j]
    
        return list(population_next_max_seq) , list(population_next_max_seq_fitness)


    def minimize_next_generation(self, population_current, population_current_fitness): 
        ...
        for i in tqdm(range(len(population_current))) :    
            min_score = np.inf
            for j in range(3*self.seq_len) : 
                if (population_next_all_predictions[3*self.seq_len*i + j] < min_score) :
                    population_next_min_seq[i] = population_next_all[3*self.seq_len*i + j]
                    population_next_min_seq_fitness[i] = population_next_all_predictions[3*self.seq_len*i + j]
                    min_score = population_next_all_predictions[3*self.seq_len*i + j]
    
        return list(population_next_min_seq) , list(population_next_min_seq_fitness)
    
    def run(self,
            MaxIter = 10, # shifting steps
            mode = "max",
            flanking=None): 
        ...
        for Iteration in range(MaxIter):
            if (mode=="max"):
                generation_neutral_seq, generation_neutral_seq_fitness = self.maximize_next_generation(self.Seqs, self.Pred)
            elif (mode=="min"):
                generation_neutral_seq, generation_neutral_seq_fitness = self.minimize_next_generation(self.Seqs, self.Pred)
        ...
        res_melt['Selection Direction'] = mode
        ...
        write_fa(self.outdir + "/ExpIter_{}_{}.txt".format(mode, MaxIter), self.Seqs)
        write_profile(self.outdir + "/ExpIter_{}_{}.csv".format(mode, MaxIter), self.Seqs, self.Pred)
        
        pdf = PdfPages(self.outdir + '/compared_with_natural_{}_{}.pdf'.format(mode, MaxIter))
        plt.figure()
        ...
        pdf = PdfPages(self.outdir+'/each_iter_distribution_{}_{}.pdf'.format(mode, MaxIter))
        plt.figure()
